...
final class ShelfPanelController: NSWindowController, NSWindowDelegate {
    ...
    private func position(_ window: NSWindow, below anchorFrame: CGRect?) {
        ...
        // Centered under the BarShelf control, on the control's own screen.
...
private struct ShelfGearMenu: NSViewRepresentable {
    ...
    final class MenuActions: NSObject {
        ...
        @objc func showMenu(_ button: NSButton) {
            ...
            let quit = menu.addItem(withTitle: "Quit BarShelf", action: #selector(quitBarShelf), keyEquivalent: "")
            quit.target = self
        ...
        @objc private func quitBarShelf() {
            coordinator?.quitApp()
        }
    }
}
